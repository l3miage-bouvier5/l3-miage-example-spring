...
import fr.uga.l3miage.example.error.ChaineCaractereVideOuNullErrorResponse;
...
import fr.uga.l3miage.example.exception.rest.ChaineCaractereVideOuNullRestException;
 ...
 * l'exception {@link IsNotTestRestException}<br>
 * Les annotations :
 * <ul>
...
@ControllerAdvice
@Slf4j
public class ChaineCaractereVideOuNullExceptionHandler {
    /**
     * Cette classe correspond au handler de l'exception
     * {@link IsNotTestRestException}.<br>
     * Ici lorsque le code va lever l'exception {@link IsNotTestRestException} alors
     * la fonction <b color="blue">handle()</b> sera appelée.<br>
     * Les annotations :
     * <ul>
     ...
     */
    @ExceptionHandler(ChaineCaractereVideOuNullRestException.class)
    public ResponseEntity<ErrorResponse> handle(HttpServletRequest httpServletRequest, Exception exception) {
        ChaineCaractereVideOuNullRestException ex = (ChaineCaractereVideOuNullRestException) exception;
        final ChaineCaractereVideOuNullErrorResponse response = ChaineCaractereVideOuNullErrorResponse
                .builder()
