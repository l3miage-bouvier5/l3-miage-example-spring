    ...
    public Miahoot createMiahoot(final CreateMiahootRequest createMiahootRequest, String userId) {
        ...
        try {
            ...
            throw new MiahootEmptyRestException(String.format(
                    "Une erreur lors de la création de l'entité Miahoot à été détecté: le miahoot avec nom = (%s)  ne contient pas de questions",
                    newMiahootEntity.getNom()), createMiahootRequest, ex);
            ...
            throw new DuplicationLabelReponsePourUneQuestionRestException(String.format(
                    "Une erreur lors de la création de l'entité Miahoot à été détecté: le miahoot avec nom = (%s) contient une ou plusieurs questions qui ont 2 réponses identiques",
                    newMiahootEntity.getNom()));
        } catch (NbReponsesVraiInvalidException ex) {
            throw new NbReponsesVraiInvalidRestException(String.format(
                    "Une erreur lors de la création de l'entité Miahoot à été détecté: le miahoot avec nom = (%s) contient une ou plusieurs questions qui n'ont aucune réponse vraie",
                    newMiahootEntity.getNom()));
        }
    }
